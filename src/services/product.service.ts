...
const createProduct = async (productData: IProduct) => {
  ...
  const productExists = await productRepository.checkProductExists(
    productData.name,
    productData.restaurantId,
  );
  if (productExists) {
    throw new ApiError(409, `${productData.name} already exists.`);
  }

  //check if the restaurant exists
  const restaurantExists = await restaurantRepository.checkRestaurantExistsById(
    productData.restaurantId.toString(),
  );
  if (!restaurantExists) {
...
const updateProduct = async (productId: string, data: IProduct) => {
  ...
  const existingRestaurant =
    await restaurantRepository.checkRestaurantExistsById(
      data.restaurantId as unknown as string,
    );
